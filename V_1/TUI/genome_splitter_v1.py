...
def split_genome(input_fasta, output_dir, num_chunks=None, chunk_size=None):
    """
    ...
    for record in records:
        ...
        for i in range(num_chunks):
            ...
            chunk_filename = os.path.join(output_dir, f"{record.id}_chunk_{i+1}.fasta")
            with open(chunk_filename, "w") as out_file:
                out_file.write(f">{record.id}_chunk_{i+1}\n{chunk_seq}\n")
...
class GenomeSplitterApp(App):
    """A simple TUI app for genome splitting."""
    ...
    def on_button_pressed(self, event):
        if event.button.id == "split":
            ...
            split_genome(input_fasta, output_dir, num_chunks=num_chunks, chunk_size=chunk_size)
            print("Genome splitting initiated!")
